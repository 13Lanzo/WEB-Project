...
export default function Chat() {
    ...
    useEffect(()=>{
        if(!myId) return; // se non ho un ID, non faccio nulla

        const token = localStorage.getItem('token'); // recupera il token JWT da localStorage
        fetch('http://localhost:5000/api/users/users', {
            headers: { 'Authorization': `Bearer ${token}`} // invia il token nell'header Authorization
        })
        .then(res => res.json())
        .then(resData => {
            if (resData.success) {
                ...
                // Mappa gli utenti in contatti con un formato adatto alla UI
                const contactsData = otherUsers.map(user => ({
                    id: user._id,
                    name:  `${user.nome} ${user.cognome}`,
                    lastMsg: 'Nessun messaggio ancora',
                    time: '',
                    active: contactToChatWith && user.id === contactToChatWith.contactId // attivo se è il contatto con cui stiamo chattando
                }));
                ...
                if (contactToChatWith && contactToChatWith.contactId) {
                    ...
                    mappedContacts[0].active = true; // se non abbiamo un contatto specifico, attiviamo il primo della lista
